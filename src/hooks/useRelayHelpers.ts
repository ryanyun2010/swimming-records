...
export function useRelayHelpers(parsedTimes: ParsedTime[], relayLegs: Record<number,RelayLeg>){
	...
	const getParsedTimesForRelay = useCallback((relayID: number): Res<ParsedTime[], Errors.NotFound> => {
	...
	const getRelayLegsForRelay = useCallback((relayID: number): Res<RelayLeg[], Errors.NotFound> => {
		const indicesAndIDs = mapping[relayID];
