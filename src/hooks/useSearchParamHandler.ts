...
export function useSearchParamHandler(
	...
	const { curMeetInfo, curSwimmerInfo, curRelayInfo } = useMemo(() => {
		...
		if (
			searchParams.get("meet_id") != null &&
			searchParams.get("meet_id")!.length > 0
		) {
			curMeetInfo = meets[parseInt(searchParams.get("meet_id")!)] ?? null;
		}

		if (
			searchParams.get("swimmer_id") != null &&
			searchParams.get("swimmer_id")!.length > 0
		) {
			curSwimmerInfo =
				swimmers[parseInt(searchParams.get("swimmer_id")!)] ?? null;
		}

		if (
			searchParams.get("relay_id") != null &&
			searchParams.get("relay_id")!.length > 0
		) {
			...
			if (relay) {
				curRelayInfo = getRelayLegsForRelay(relay.id)
					.andThen((relayLegs: RelayLeg[]) => {
						...
						return ok(
							relayLegs.map((leg) => swimmers[leg.swimmer_id])
						);
					})
					...
					.andThen((swimmer_names: string[]) => {
						...
						if (!event)
							return err(
								new Errors.NotFound(
									`No event found with ID ${relay.event_id} `
								)
							);
						if (!meets[relay.meet_id])
							return err(
								new Errors.NotFound(
									`No meet found with ID ${relay.meet_id}`
								)
							);
						return ok({
							id,
					...
					.match(
						...
						(err) => {
							console.error(
								`Failed to load relay info for relay ID ${id}:`,
								err.toString()
							);
							return null;
						}
