		...
		if (searchParams.get("relay_id") != null && searchParams.get("relay_id")!.length > 0) {
			...
			if (relay) {
				...
				.andThen((swimmer_names: string[]) => {
					...
					return ok({id, swimmer_names, date: meets[relay.meet_id].date ,event: event.name});
				})
				.match(
					(d) => d,
					(err) => {console.error(`Failed to load relay info for relay ID ${id}:`, err.toString()); return null;}
				);
